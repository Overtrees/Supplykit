...
export default function RulesPage() {
  ...
  const IS={width:'100%',padding:'6px 8px',fontSize:16,border:'1px solid var(--border)',borderRadius:6,marginTop:4,outline:'none',background:'var(--card)',boxSizing:'border-box'}
...
const pc=j=>{try{const c=JSON.parse(j);const rt=c.rightType||(LF.find(x=>x.v===c.right)?'field':(typeof c.right==='string'&&!c.right.replace('.','').match(/^\d+$/)?'text':'number'));return{left:c.left||'inv.available_qty',op:c.op||'<',right:c.right||'inv.safety_qty',rightType:rt}}catch{return{left:'inv.available_qty',op:'<',right:'inv.safety_qty',rightType:'field'}}}
  ...
  return <div className='card'>
    ...
    {tab==='rules'&&<>
      {editing!==null&&<div style={{background:'var(--bg)',border:'1px solid var(--border)',borderRadius:12,padding:16,marginBottom:16}}>
      ...
      {rules.map(rule => {
        ...
        return <div key={rule.id} style={{padding:'10px 14px',border:'1px solid var(--border)',borderRadius:10,marginBottom:6,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
    ...
    {tab==='params'&&<div>
      ...
      {seasons.map((s,i)=><div key={s.key||i} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',border:'1px solid var(--border)',borderRadius:10,marginBottom:6}}>
        <input value={s.name} onChange={e=>setSeasons(p=>p.map((x,j)=>j===i?{...x,name:e.target.value}:x))} placeholder='活动名称' style={{width:110,fontSize:16,padding:'5px 8px',border:'1px solid var(--border)',borderRadius:6,outline:'none'}}/>
        <span className='small muted' style={{fontSize:11}}>×</span>
        <input type='number' value={s.factor} onChange={e=>setSeasons(p=>p.map((x,j)=>j===i?{...x,factor:parseFloat(e.target.value)||1}:x))} step='0.1' min='1' max='3' style={{width:70,fontSize:16,padding:'5px 8px',border:'1px solid var(--border)',borderRadius:6,outline:'none'}}/>
