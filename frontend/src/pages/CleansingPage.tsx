  ...
  return <div className="card">
    ...
    {s === 1 && <div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center',flexWrap:'wrap'}}>
        <select id="tmplSelect" style={{flex:1,fontSize:16,padding:'6px 8px',border:'1px solid var(--border)',borderRadius:6,minWidth:140}}>
        ...
        <button onClick={()=>{const s=document.getElementById('tmplSelect');if(s.value)try{setMp(JSON.parse(s.value))}catch(e){}}} style={{padding:'6px 14px',fontSize:12,border:'1px solid var(--border)',borderRadius:6,background:'var(--card)',cursor:'pointer'}}>应用</button>
        <input id="tmplName" placeholder="新模板名称" style={{width:120,fontSize:16,padding:'6px 8px',border:'1px solid var(--border)',borderRadius:6,outline:'none'}}/>
      ...
      {Array.isArray(cf) && <div style={{marginBottom:10,border:'1px solid var(--border)',borderRadius:12,padding:12,background:'var(--bg)'}}>
        ...
        {cf.map((f,i) => <div key={i} style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
          <input value={f.l} onChange={e=>{const v=e.target.value;setCf(p=>p.map((x,k)=>k===i?{...x,l:v}:x))}} placeholder="字段名" style={{flex:1,fontSize:16,padding:'5px 8px',border:'1px solid var(--border)',borderRadius:6,outline:'none'}}/>
          <select value={f.tp} onChange={e=>{const v=e.target.value;setCf(p=>p.map((x,k)=>k===i?{...x,tp:v}:x))}} style={{fontSize:14,padding:'5px',border:'1px solid var(--border)',borderRadius:6}}>
      ...
      {cols.map(c => {
        ...
        return (<div key={c.name} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 10px',border:'1px solid var(--border)',borderRadius:10,marginBottom:4}}>
        ...
        <select value={mp[c.name]?.target || ''} onChange={e=>{const v=e.target.value;setMp(p=>({...p,[c.name]:{target:v,type:'string'}}))}}
          style={{fontSize:13,padding:'5px 8px',border:'1px solid var(--border)',borderRadius:6,flex:1,minWidth:120,background:'var(--card)'}}>
