...
<div className='section-title' style={{display:'flex',flexWrap:'wrap',gap:6,alignItems:'center'}}>
  <span>商品管理 <span className='small muted'>{t("common.total")} {list.length} {t("ommon.items")}</span></span>
</div>
{fl.length===0?<EmptyState icon='tag' title={s?'{t("roduct.empty_matched")}':'{t("roduct.empty")}'} desc={s?'换个关键词试试':'通过清洗页导入商品数据'}/>:<div style={{overflowX:"auto"}}>
