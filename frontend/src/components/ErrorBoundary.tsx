    ...
    if (this.state.err) {
      return (
        <div style={{ padding: 24, background: 'rgba(225,29,72,0.08)', border: '1px solid rgba(225,29,72,0.3)', borderRadius: 32, margin: 12 }}>
          ...
          <button onClick={() => this.setState({ err: null })} style={{ padding: '6px 14px', fontSize: 12, border: '1px solid rgba(225,29,72,0.3)', borderRadius: 32, background: 'var(--card)', color: 'var(--danger)', cursor: 'pointer' }}>{t("ommon.retry")}</button>
        </div>
      )
    }
