...
"""
import os, sys
os.environ['SQLITE_PATH'] = os.path.join(os.path.dirname(__file__), '..', '.test_replenish_order.db')
...
def setup_module():
    global client, db, invalidate_cache, unwrap
    ...
    products = [
        {"sku":"SKU-FULL","product_name":"库存充足品","store":"测试店","box_qty":12,"status":"active","barcode":"BAR-FULL","channel":"jd"},
        {"sku":"SKU-NEED","product_name":"需要补货品","store":"测试店","box_qty":12,"status":"active","barcode":"BAR-NEED","channel":"jd"},
    ]
