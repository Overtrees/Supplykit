          ...
          {purchaseLoading ? (
            ...
            <div style={{overflowY:"auto",overflowX:"hidden",maxHeight:"calc(100vh - 180px)"}}>
              ...
              <div style={{overflowX:"auto"}}><table>
                ...
                <thead><tr>{purchaseVisCols.map(id => {const col = PURCHASE_COLS.find(c => c.id === id); return col ? <th style={{position:"sticky",top:0,background:"var(--card)",zIndex:1}} key={col.id} style={{whiteSpace:'nowrap',fontSize:11,padding:'8px 4px'}}>{col.label}</th> : null})}</tr></thead>
                <tbody>
            ...
            <>
              <div style={{overflowY:"auto",overflowX:"hidden",maxHeight:"calc(100vh - 180px)"}}>
                ...
                <div style={{overflowX:"auto"}}><table>
                  ...
                  <thead><tr>{slowVisCols.map(id => {const col = SLOW_COLS.find(c => c.id === id); return col ? <th style={{position:"sticky",top:0,background:"var(--card)",zIndex:1}} key={col.id} style={{whiteSpace:'nowrap',fontSize:11,padding:'8px 4px'}}>{col.label}</th> : null})}</tr></thead>
                  <tbody>
