...
| 模式 | 一句话 | 适合谁 |
|------|--------|--------|
| **BBCC** | 全国一盘棋，盯 B 仓库存能不能撑住 C 仓消耗 | 京东入仓商家 |
| **传统** | 按仓逐条算，各仓独立补货 | 自有仓发货，多仓调拨 |

### 📊 看板一页尽览

- GMV 趋势 + 店铺分布 + 订单阶段转化漏斗
- 库存健康度（自有仓/平台仓独立评分）
- **濒临断货 TOP10** — 按可撑天数排序，哪个 SKU 最紧急一目了然
- 规则引擎实时告警

### 🧹 数据清洗，告别 Excel 手工

- 上传 Excel/CSV → 自动识别 30+ 种中文列名 → 可视化映射
- 支持模板保存（京东采购单、商智报表等），下次直接复用
- 异步导入 + 进度追踪，大文件不卡界面
- 去重保护 + 异常记录，发现问题可追溯

### ⚙️ 规则引擎，自动监控

- 预设 4 条规则：低库存预警 / 紧急补货 / 超卖保护 / 滞销识别
...
| 页面 | 核心能力 |
|------|---------|
| 📊 **看板** | GMV趋势 / 店铺GMV / 漏斗转化 / 库存健康度 / 濒临断货TOP10 / 实时告警 |
| 💡 **补货建议** | BBCC全国汇总 / 传统按仓 / 三窗口滚动日销 / B仓超储预警 / 一键标记操作 |
| 📦 **采购建议** | 14+28天融合日销 / 系统总库存视角 / MOQ兜底 / 导出Excel |
| 🧹 **数据清洗** | 智能列名匹配 / 字段映射 / 模板复用 / 异步导入 / 自定义字段 / 异常追溯 |
| ⚙️ **规则引擎** | 事件驱动 / 条件可视化编辑 / 百分比比较 / 告警模板变量 |
| 📋 **订单明细** | 分页 / 搜索 / 状态筛选 |
| 📦 **进销存台账** | 出入库记录 / 周转计算 |
| 🏷️ **商品/供应商** | CRUD / 搜索 |
| ⚠️ **异常记录** | 数据质量日志 |
