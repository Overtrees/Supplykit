    ...
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
      <div style={{ display:'flex', gap:4 }}>
        {['today','week','month'].map(k => (
          <button key={k} onClick={() => setPeriodTab(k)} className={`btn btn-ghost`} style={{fontSize:12,background:periodTab===k?'rgba(128,128,128,0.2)':'transparent',color:periodTab===k?'var(--primary)':'',borderColor:periodTab===k?'transparent':''}}>{periodLabel[k]}</button>
        ))}
      </div>
        ...
        {dashboard?.health_index && (()=>{
          ...
          return <>
            ...
            <div style={{ display:'flex', gap:4, marginBottom:8 }}>
              <span onClick={()=>{localStorage.setItem('health_tab','own');setHealthTab('own')}} className="btn btn-ghost" style={{fontSize:12,padding:'2px 12px',background:healthTab==='own'?'rgba(128,128,128,0.2)':'transparent',color:healthTab==='own'?'var(--primary)':'',borderColor:healthTab==='own'?'transparent':''}}>自有仓</span>
              <span onClick={()=>{localStorage.setItem('health_tab','platform');setHealthTab('platform')}} className="btn btn-ghost" style={{fontSize:12,padding:'2px 12px',background:healthTab==='platform'?'rgba(128,128,128,0.2)':'transparent',color:healthTab==='platform'?'var(--primary)':'',borderColor:healthTab==='platform'?'transparent':''}}>平台仓</span>
            </div>
