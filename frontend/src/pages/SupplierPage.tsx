...
<table style={{minWidth:440}}><thead><tr>{['编号','名称','联系人','手机','评分'].map(h=><th key={h} style={{whiteSpace:'nowrap',padding:'8px 6px'}}>{h}</th>)}</tr></thead>
<tbody>{fl.map(x=><tr key={x.id}><td className='mono col-sku' style={{minWidth:80}}>{x.supplier_code||x.code}</td><td className='col-name' style={{minWidth:100}}>{x.supplier_name}</td><td className='col-store' style={{minWidth:72}}>{x.contact_person}</td><td className='col-store' style={{minWidth:72}}>{x.contact_phone||x.phone}</td><td className='col-price' style={{minWidth:60}}><span className={'pill '+(x.score>3?'success':'warning')}>{x.score}/5</span></td></tr>)}</tbody></table></div>}</div>}
