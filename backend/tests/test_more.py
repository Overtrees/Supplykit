...
import os, sys
_db_path = os.path.join(os.path.dirname(__file__), '..', 'test_more.db')
...
for t in ['products','orders','inventory','alerts']:
...
# 种子数据
for p in [{"sku":"SKU-T01","product_name":"测试商品A","store":"测试店","box_qty":12,"status":"active"}]:
    db.table("products").insert(p).execute()
for o in [("O-1","SKU-T01",50,1),("O-2","SKU-T01",30,3),("O-3","SKU-T01",20,7)]:
    db.table("orders").insert({"order_no":o[0],"sku":o[1],"quantity":o[2],"ordered_at":(today-timedelta(days=o[3])).strftime("%Y-%m-%d"),"order_status":"已完成"}).execute()
for i in [("SKU-T01","北京","platform",100,50,200),("SKU-T01","B-华东","platform_b",300,0,0)]:
    db.table("inventory").insert({"sku":i[0],"warehouse":i[1],"warehouse_type":i[2],"available_qty":i[3],"in_transit_qty":i[4],"safety_qty":i[5],"product_name":"","store":"测试店"}).execute()
for k,v in [("b_to_c_days","3"),("c_safety_days","0"),("turnover_warning_90","90")]:
    db.table("replenishment_config").upsert({"key":k,"value":v})
db.table("alerts").insert({"alert_type":"low_stock","title":"测试告警","description":"可用<安全线","severity":"warning","status":"active","related_sku":"SKU-T01","source":"test"}).execute()
...
class TestInventory:
    ...
    def test_create(self):
        r = client.post("/api/inventory", json={"sku":"SKU-T01","warehouse":"广州","warehouse_type":"platform","available_qty":50})
        assert r.status_code == 200
...
def teardown_module():
    try:
        ...
        if hasattr(_local, 'conn') and _local.conn: _local.conn.close()
        os.unlink(_db_path)
    except: pass