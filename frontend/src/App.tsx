...
export default function App() {
  ...
  const navigateTo = (p: string) => { setPage(p); if (p === 'dash') { clearCache(); useAppStore.getState().loadAll().catch(() => {}); setTimeout(function() { api.get('/api/dashboard/summary?channel=jd&refresh=1').then(function(r) { useAppStore.setState({dashboard: r.data || null}) }).catch(() => {}) }, 2000) } }
