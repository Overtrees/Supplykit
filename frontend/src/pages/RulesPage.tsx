...
export default function RulesPage() {
  ...
  const loadAll = async (ch) => { setLoading(true); const c=ch||globalChannel; const m=c!=='jd'?'traditional':(cfg.replenishment_mode||'bbcc'); await load(c); try{const flat=await api.get('/api/replenishment-config?channel='+c);if(flat.data)setCfg(p=>({...p,...flat.data,replenishment_mode:m}))}catch(e){} await loadCfg(m,c); await loadSeasons(m,c); setLoading(false) }
