...
export default function RulesPage() {
  ...
  const loadAll = async (ch) => { setLoading(true); const c=ch||globalChannel; const savedMode=(()=>{try{return localStorage.getItem('c_replen_mode_'+c)}catch{return null}})(); const m=c!=='jd'?'traditional':(savedMode||'bbcc'); clearCache(); clearInflight(); try{const _s=localStorage.getItem('c_supplier_'+c);if(_s)setSelectedSupplier(_s)}catch{} /* 并行加载(串行9s→并行3s) */ await Promise.all([ (async()=>{try{await load(c)}catch(e){}})(), (async()=>{try{const flat=await api.get('/api/replenishment-config?channel='+c);if(flat.data)setCfg(p=>({...p,...flat.data,replenishment_mode:m}))}catch(e){}})(), (async()=>{try{const r=await api.get('/api/replenishment-config?mode='+m+'&channel='+c);if(r.data&&Object.keys(r.data).length>0)setCfg(p=>({...p,...r.data,replenishment_mode:m}))}catch(e){}})(), (async()=>{try{const r=await api.get('/api/replenishment-config/seasons?mode='+m+'&channel='+c);setSeasons(r.data||[])}catch(e){}})(), (async()=>{try{const sr=await api.get('/api/suppliers?channel='+c);if(sr.data)setSuppliers(sr.data.map(x=>x.supplier_code).filter(Boolean))}catch(e){}})() ]); setLoading(false) }
