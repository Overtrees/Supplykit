                  ...
                  {showColPicker && <div style={{position:'absolute',top:'100%',right:0,zIndex:10,background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,padding:6,minWidth:180,boxShadow:'0 4px 12px rgba(0,0,0,0.15)'}}>
                    ...
                    {currentCols.map((col, idx) => {
                      ...
                      return <div key={col.id} draggable={isVis?true:undefined}
                        ...
                        onDragOver={isVis?e=>{e.preventDefault();e.currentTarget.style.borderTop='2px solid var(--primary)';const from=e.currentTarget.parentNode._dragId;if(from&&from!==col.id){const now=Date.now();if(!e.currentTarget._lastDrag||now-e.currentTarget._lastDrag>100){e.currentTarget._lastDrag=now;const nxt=visCols.filter(c=>c!==from);const toIdx=nxt.indexOf(col.id);nxt.splice(toIdx,0,from);setVisCols(nxt);localStorage.setItem(colKey(replenMode),JSON.stringify(nxt))}}}:undefined}
