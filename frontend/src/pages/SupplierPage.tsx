...
<input value={s} onChange={e=>setS(e.target.value)} placeholder='搜索供应商...' style={{width:'100%',padding:'8px 12px',fontSize:13,border:'1px solid #e2e8f0',borderRadius:8,marginBottom:12,outline:'none',boxSizing:'border-box'}}/>
...
<tbody>{fl.map(x=><tr key={x.id}><td className='mono' style={{fontSize:12}}>{x.code}</td><td>{x.supplier_name}</td><td>{x.contact_person}</td><td>{x.phone}</td><td><span className={'pill '+(x.score>3?'success':'warning')}>{x.score}/5</span></td></tr>)}</tbody></table></div>}</div>}
