  ...
  return <>
    ...
    <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:9999,padding:'0 16px 40px',paddingBottom:'calc(40px + env(safe-area-inset-bottom, 20px))',animation:'slideUp 0.25s ease'}}>
      <div style={{background:'var(--card)',borderRadius:26,padding:24,textAlign:'center',marginBottom:8}}>
        ...
        <button onClick={onConfirm} style={{width:'100%',padding:'14px',background:'var(--danger)',color:'var(--card)',border:'none',borderRadius:26,cursor:'pointer',fontSize:16,fontWeight:600}}>{confirmLabel}</button>
      </div>
      <button onClick={onCancel} style={{width:'100%',padding:'14px',background:'var(--card)',border:'none',borderRadius:26,cursor:'pointer',fontSize:16,fontWeight:600,color:'var(--primary)',boxShadow:'0 1px 4px rgba(0,0,0,0.08)'}}>{cancelLabel}</button>
    </div>
